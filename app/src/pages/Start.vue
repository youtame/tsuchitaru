<template>
  <v-container class="start-section py-8">
    ...
    <v-form @submit.prevent="submitForm">
      <v-row>
        <v-col cols="12" md="4">
          <v-select v-model="weight" :items="weightOptions" label="あなたの体重" suffix="kg" item-title="title" item-value="value" variant="outlined" required></v-select>
        </v-col>
        <v-col cols="12" md="4">
          <v-select v-model="speed" :items="speedOptions" label="歩く速度" item-title="label" item-value="value" variant="outlined" required></v-select>
        </v-col>
        <v-col cols="12" md="4">
          <v-select v-model="targetCalories" :items="calorieOptions" label="目標消費カロリー" item-title="label" item-value="value" variant="outlined" required></v-select>
        </v-col>
      </v-row>
      ...
      <div class="map-selection-ui mb-4">
        <v-row dense>
          <v-col cols="12" sm="6">
            ...
            >
              <div class="d-flex align-center mb-1">
                <v-icon icon="mdi-map-marker" :color="startCoords ? 'orange-darken-2' : 'grey'" class="mr-2"></v-icon>
                <span class="text-subtitle-2 font-weight-bold">STEP 1. 出発地の設定</span>
              </div>
              <div v-if="startCoords" class="d-flex align-center justify-space-between">
                <span class="text-caption text-grey-darken-2">座標: {{ startCoords.lng.toFixed(4) }}, {{ startCoords.lat.toFixed(4) }}</span>
                <v-btn size="x-small" color="orange-darken-2" variant="text" @click="resetStart">やり直す</v-btn>
              </div>
              <div v-else class="text-caption text-grey">地図の好きな場所をクリックしてください</div>
            </v-card>
          </v-col>
          ...
          <v-col cols="12" sm="6">
            ...
            >
              <div class="d-flex align-center mb-1">
                <v-icon icon="mdi-bus" :color="selectedGoal ? 'teal-darken-2' : 'grey'" class="mr-2"></v-icon>
                <span class="text-subtitle-2 font-weight-bold">STEP 2. ゴールのバス停</span>
              </div>
              <div v-if="selectedGoal" class="d-flex align-center justify-space-between">
                <span class="text-body-2 font-weight-bold text-teal-darken-4">{{ selectedGoal.n }}</span>
                <v-btn size="x-small" color="teal-darken-2" variant="text" @click="selectedGoal = null">変更</v-btn>
              </div>
              <div v-else class="text-caption text-grey">地図上の緑色のアイコンを選択してください</div>
            </v-card>
          </v-col>
        </v-row>
      </div>
      ...
      <div class="map-wrapper mb-6">
        <div id="map" class="map-container border-md shadow-sm" style="height: 450px;"></div>
        <transition name="fade">
          <div v-if="mapZoom < 12" class="zoom-overlay d-flex align-center justify-center">
            <v-chip color="black" class="px-4">もっとズームするとバス停が表示されます</v-chip>
          </div>
        </transition>
      </div>
...
interface Coords { lng: number; lat: number; }
interface BusStop { id: string | number; n: string; lat: number; lng: number; }
...
export default defineComponent({
  ...
  computed: {
    ...
    isReady(): boolean {
      return !!(this.weight && this.speed && this.targetCalories && this.startCoords && this.selectedGoal);
    }
  },
  mounted() {
    this.initMap();
  ...
  methods: {
    ...
    updateStartMarker() {
      ...
      if (this.startMarker) {
        this.startMarker.setLngLat([this.startCoords.lng, this.startCoords.lat]);
      } else {
